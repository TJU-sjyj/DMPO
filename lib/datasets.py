...
def build_dataset(is_train, args, folder_name=None,is_individual_prompt=False):
    ...
    if args.data_set == 'CIFAR10':
        ...
        dataset = dogs(root=args.data_path, train=is_train, transform=transform)
        nb_classes = 120
    elif args.data_set == 'nabirds':
        dataset = NABirds(root=args.data_path, train=is_train, transform=transform)
        nb_classes = 555
    elif args.data_set == 'cub2011':
        dataset = Cub2011(root=args.data_path, train=is_train, transform=transform)
        nb_classes = 200
    elif args.data_set == 'oxford_flowers':
        split = "train" if is_train else "validation"
        if os.path.isdir(os.path.join(args.data_path, split)):
            root = os.path.join(args.data_path, split)
        else:
            if os.path.isdir(args.data_path):
                root = _search_split(args.data_path, split)
        dataset = ImageDataset(root, parser=args.data_set, class_map='', load_bytes=False, transform=transform)
        nb_classes = 102
