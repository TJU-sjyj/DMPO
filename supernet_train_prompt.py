...
from supernet_engine_prompt import train_one_epoch, evaluate, exit_evaluate
...
from loss_schedule.Lambda import Lambda
from loss_schedule.alpha import *
...
def get_args_parser():
    ...
    parser.add_argument('--mode',
                        type=str,
                        default='super',
                        choices=['super', 'vp', 'retrain', 'search'],
    ...
    parser.add_argument('--val_interval',
                        default=10,
                        type=int,
                        help='validataion interval')

    parser.add_argument('--drop_rate_LoRA', type=float, default=0.2)
...
def main(args):
    ...
    dataset_train, args.nb_classes = build_dataset(
        is_train=True,
        args=args,
        is_individual_prompt=(args.is_visual_prompt_tuning or args.is_adapter
                              or args.is_LoRA or args.is_prefix))
    ...
    pin_mem = False if args.data_set in {"dsprites_loc", "dsprites_ori"} else args.pin_mem
    ...
    if args.resume:
        if 'pth' in args.resume:
            if args.nb_classes != model.head1.weight.shape[0]:
                model.reset_classifier(args.nb_classes, use_mp=None, use_mlp=None, scale=1)
            incompatible_keys = load_checkpoint(model,
                                                args.resume,
                                                strict=False)
    ...
    idx = args.output_dir.find("_lr")
    ds = args.output_dir[:idx]
    cond = args.output_dir[idx + 1:]
    output_dir = f"{ds}/{cond}/{args.dirname}"
    output_dir = Path(output_dir)
    
    if (args.distributed and global_rank == 0) or not args.distributed:
    ...
    if args.mode == 'retrain' and "RETRAIN" in cfg:
        retrain_config = {
            'visual_prompt_dim': cfg.RETRAIN.VISUAL_PROMPT_DIM,
            'lora_dim': cfg.RETRAIN.LORA_DIM,
            'adapter_dim': cfg.RETRAIN.ADAPTER_DIM,
    ...
    gamma_dict = {
        "type": "Linear",
        "args": {
            "End": [1.5, 1.0, 0.2475, 0.2675],
            "Start": [0.01, 0.01, 1.0, 2.0],
            "Epochs": args.epochs
        }
    }
    lmda = Lambda(gamma_dict=gamma_dict)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.val_interval == 0 or epoch == args.epochs - 1:
            ...
            if acc > max_accuracy:
                ...
                # save(checkpoins, acc, epoch, output_dir)
            if acc70 > best_70:
                best_70 = acc70
                best_70_ep = epoch
            if acc50 > best_50:
        ...
        if epoch % args.val_interval == 0 or epoch == args.epochs - 1:
            log(output_dir, acc, epoch)
            log70(output_dir, acc70, epoch)
            log50(output_dir, acc50, epoch)
    
    if args.output_dir and utils.is_main_process():
        log_stats = {
                'best_acc': max_accuracy, 'beat_epoch': best_ep,
                'best_70': best_70, 'beat_70_epoch': best_70_ep, 'best_50': best_50, 'beat_50_epoch': best_50_ep
            }
        txt = f"best_acc--{best_ep}: {max_accuracy:.4f}\nbest_acc50--{best_50_ep}: {best_50:.4f}\nbest_acc70--{best_70_ep}: {best_70:.4f}\n"
        with (output_dir / "log.txt").open("a") as f:
            f.write(json.dumps(log_stats) + "\n")
            f.write(txt)
