...
class Covariance(nn.Module):
    ...
    def _cross_cov(self, x1, x2):
        ...
        y = (1.0 / N1) * (x1.bmm(x2.transpose(1, 2)))
        return y
...
class Moment_Probing_ViT(nn.Module):
    ...
    def forward(self, cls_token, x):
        ...
        if self.cross_type == "n/2":
            cov_list = self.cov(
                self.ln(heads[0]), self.ln(heads[self.num_heads // 2])
            ).unsqueeze(1)
            cov_list = self._l2norm(cov_list)

            for i in range(1, self.num_heads // 2):
                cov = self.cov(
                    self.ln(heads[i]), self.ln(heads[i + self.num_heads // 2])
                ).unsqueeze(1)
...
class Moment_Probing_CNN(nn.Module):
        ...
        self.cls_head = nn.Sequential(
            OrderedDict(
                [
                    ...
                    (
                        "fc",
                        nn.Linear(self.in_dim, num_classes)
                        if num_classes > 0
                        else nn.Identity(),
                    ),
                ]
    ...
    def forward(self, x):
        ...
        if self.cross_type == "one":
            ...
            cov_list = self.cov(
                self.bn(heads[0]), self.bn(heads[self.num_heads // 2])
            ).unsqueeze(1)
            cov_list = self._l2norm(cov_list)

            for i in range(1, self.num_heads // 2):
                cov = self.cov(
                    self.bn(heads[i]), self.bn(heads[i + self.num_heads // 2])
                ).unsqueeze(1)
                cov_list = self._l2norm(cov_list)
                cov_list = torch.cat([cov_list, cov], dim=1)
        else:
