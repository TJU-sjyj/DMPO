...
from model.MP import Moment_Probing_ViT
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='',super_prompt_tuning_dim=1024,super_LoRA_dim=1024,super_adapter_dim=1024,super_prefix_dim=1024,drop_rate_LoRA=0,drop_rate_prompt=0,drop_rate_adapter=0, IS_not_position_VPT=False, beta_list=[1.0, 1.0, 1.0, 1.0], beta_train=False):
        """
        Args:
        ...
        #     ]))
        # else:
        self.pre_logits1 = nn.Identity()
        self.pre_logits2 = nn.Identity()
        self.pre_logits3 = nn.Identity()
        self.pre_logits4 = nn.Identity()
        ...
        self.head3 = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
        self.head4 = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
        self.head_dist = None
        if distilled:
        ...
        if not beta_train:
            ...
            self.beta4 = nn.Parameter(torch.tensor(1.0))
        else:
            self.beta1 = nn.Parameter(torch.tensor(beta_list[0]))
            self.beta2 = nn.Parameter(torch.tensor(beta_list[1]))
            self.beta3 = nn.Parameter(torch.tensor(beta_list[2]))
            self.beta4 = nn.Parameter(torch.tensor(beta_list[3]))
    ...
    def freeze_stages(self):
        ...
        for name,param in self.named_parameters():
            ...
            if 'beta' in name:
                param.requires_grad = self.beta_train
        ...
        print('parameters:',total_para_nums,'adapter',adapter_para_nums,'LoRA',LoRA_para_nums,'prompt',vp_para_nums,'head',head_para_nums)
    ...
    def reset_classifier(self, num_classes, global_pool='', use_mp=None, use_mlp=None, scale=1):
        self.num_classes = num_classes
        self.use_mp = use_mp
        
        def get_cls(dim, idx):
            if use_mp is None or not use_mp[idx]:
                return nn.Linear(dim, num_classes)
            else:
                return Moment_Probing_ViT(in_dim=dim, num_classes=num_classes)
        
        heads = []
        for idx in range(4):
            head = []
            if use_mlp is None or use_mlp[idx] == 1:
                head.append(get_cls(dim=self.embed_dim, idx=idx))
            else:
                head.append(nn.Linear(self.embed_dim, self.embed_dim // scale))
                head.append(nn.GELU())
                for _ in range(use_mlp[idx] - 2):
                    head.append(nn.Linear(self.embed_dim // scale, self.embed_dim // scale))
                    head.append(nn.GELU())
                head.append(get_cls(dim=self.embed_dim // scale, idx=idx))
    ...
    def set_sample_config(self, config: dict):
        ...
        for i, blocks in enumerate(self.blocks):
            ...
            blocks.set_sample_config(sample_prompt_tuning_dim=self.sample_prompt_tuning_dim[i],
                                     ...
                                     sample_last_prompt_tuning_dim = self.sample_prompt_tuning_dim[i-1] if i>0 else self.sample_prompt_tuning_dim[0],
    ...
    def get_sampled_params_numel(self, config):
        ...
        if self.sample_prompt_tuning_dim[0] == 0:
            ...
            if not self.IS_not_position_VPT:
                sample_prompt_tuning_param = self.visual_prompt_token[:,:self.sample_prompt_tuning_dim[0],:].numel() + self.visual_prompt_token_pos_embed.numel()
            else:
                sample_prompt_tuning_param = self.visual_prompt_token[:,:self.sample_prompt_tuning_dim[0],:].numel()                
    ...
    def forward_features(self, x):
        ...
        outs_x = list()
        x1 = self.norm_1(X1)
        y1 = self.pre_logits1(x1[:, 0])
        outs_x.append(y1)
        x2 = self.norm_2(X2)
        y2 = self.pre_logits2(x2[:, 0])
        outs_x.append(y2)
        x3 = self.norm_3(X3)
        y3 = self.pre_logits3(x3[:, 0])
        outs_x.append(y3)
        x4 = self.norm_4(X4)
        y4 = self.pre_logits4(x4[:, 0])
        outs_x.append(y4)
        X = [X1, X2, X3, X4]
    ...
    def forward(self, x):
        outs_x, x = self.forward_features(x)
        x1, x2, x3, x4 = outs_x
        xx1, xx2, xx3, xx4 = x
        outs = list()
        
        if self.use_mp is None or not self.use_mp[0]:
            x1 = self.beta1 * self.head1(x1)
        elif self.use_mp[0]:
            x1 = self.beta1 * self.head1(x1, xx1)
        outs.append(x1)
        if self.use_mp is None or not self.use_mp[1]:
            x2 = self.beta2 * self.head2(x2)
        elif self.use_mp[1]:
            x2 = self.beta2 * self.head2(x2, xx2)
        outs.append(x2)
        if self.use_mp is None or not self.use_mp[2]:
            x3 = self.beta3 * self.head3(x3)
        elif self.use_mp[2]:
            x3 = self.beta3 * self.head3(x3, xx3)    
        outs.append(x3)
        if self.use_mp is None or not self.use_mp[3]:
            x4 = self.beta4 * self.head4(x4)
        elif self.use_mp[3]:
            x4 = self.beta4 * self.head4(x4, xx4)
        outs.append(x4)
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = ''):
    ...
    model.norm_1.weight.copy_(_n2p(w[f'{prefix}Transformer/encoder_norm/scale']))
    model.norm_1.bias.copy_(_n2p(w[f'{prefix}Transformer/encoder_norm/bias']))
    model.norm_2.weight.copy_(_n2p(w[f'{prefix}Transformer/encoder_norm/scale']))
    model.norm_2.bias.copy_(_n2p(w[f'{prefix}Transformer/encoder_norm/bias']))
    model.norm_3.weight.copy_(_n2p(w[f'{prefix}Transformer/encoder_norm/scale']))
    model.norm_3.bias.copy_(_n2p(w[f'{prefix}Transformer/encoder_norm/bias']))
    model.norm_4.weight.copy_(_n2p(w[f'{prefix}Transformer/encoder_norm/scale']))
    model.norm_4.bias.copy_(_n2p(w[f'{prefix}Transformer/encoder_norm/bias']))
    if isinstance(model.head1, nn.Linear) and model.head1.bias.shape[0] == w[f'{prefix}head/bias'].shape[-1]:
        model.head1.weight.copy_(_n2p(w[f'{prefix}head/kernel']))
        model.head1.bias.copy_(_n2p(w[f'{prefix}head/bias']))
        model.head2.weight.copy_(_n2p(w[f'{prefix}head/kernel']))
        model.head2.bias.copy_(_n2p(w[f'{prefix}head/bias']))
        model.head3.weight.copy_(_n2p(w[f'{prefix}head/kernel']))
        model.head3.bias.copy_(_n2p(w[f'{prefix}head/bias']))
        model.head4.weight.copy_(_n2p(w[f'{prefix}head/kernel']))
        model.head4.bias.copy_(_n2p(w[f'{prefix}head/bias']))
        
    if isinstance(getattr(model.pre_logits1, 'fc', None), nn.Linear) and isinstance(getattr(model.pre_logits2, 'fc', None), nn.Linear) and isinstance(getattr(model.pre_logits3, 'fc', None), nn.Linear) and isinstance(getattr(model.pre_logits4, 'fc', None), nn.Linear) and f'{prefix}pre_logits/bias' in w:
        model.pre_logits1.fc.weight.copy_(_n2p(w[f'{prefix}pre_logits/kernel']))
        model.pre_logits1.fc.bias.copy_(_n2p(w[f'{prefix}pre_logits/bias']))
        model.pre_logits2.fc.weight.copy_(_n2p(w[f'{prefix}pre_logits/kernel']))
        model.pre_logits2.fc.bias.copy_(_n2p(w[f'{prefix}pre_logits/bias']))
        model.pre_logits3.fc.weight.copy_(_n2p(w[f'{prefix}pre_logits/kernel']))
        model.pre_logits3.fc.bias.copy_(_n2p(w[f'{prefix}pre_logits/bias']))
        model.pre_logits4.fc.weight.copy_(_n2p(w[f'{prefix}pre_logits/kernel']))
        model.pre_logits4.fc.bias.copy_(_n2p(w[f'{prefix}pre_logits/bias']))
